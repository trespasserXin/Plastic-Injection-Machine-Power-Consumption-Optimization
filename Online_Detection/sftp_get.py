    ...
    try:
        ...
        if new_file_detected:
            ...
            if df_201.empty:
                ...
                #                 .format(last_file))
                return _prev_file, _end_ts, False, motor_sum, last_file
            ...
            # print("Missing cols:", missing_col)
            ...
            # Corner Case: raw df is not empty, but all timestamps are before the expected start time
            # In this case, we just treat as no data
            next_start_ts = _end_ts + pd.Timedelta(minutes=1)
            # print('Next start ts ',next_start_ts)
            invalid_ts = reformat_201.index < next_start_ts
            # print('Before invalid ',reformat_201.head())
            reformat_201 = reformat_201.loc[~invalid_ts,:]
            # print('After invalid ',reformat_201.head(), ~invalid_ts)
            if reformat_201.empty:
                print("No data in the last 15 minutes")
                # df_201_empty = no_run_patch(df, label_201)
                motor_sum = no_run_patch(_end_ts)
                _end_ts = motor_sum.index[-1]
            ...
            if file_start_ts > _end_ts + pd.Timedelta(minutes=1):
                print("File actual start later than expected, filling gaps using zeros")
